                ...
                <div class="col-md-8">
                    <div class="card">
                        <div class="card-header">
                            {{ __('แก้ไขจังหวัด') }}
                            <a href="{{ route('states.index') }}" class="float-right">ย้อนกลับ</a>
                        </div>
                            ...
                            <form method="POST" action="{{ route('states.update', $state->id) }}">
                                ...
                                <div class="form-group row">
                                    <label for="country_code"
                                        class="col-md-4 col-form-label text-md-right">{{ __('ประเทศ') }}</label>
                                ...
                                <div class="form-group row">
                                    <label for="name"
                                        class="col-md-4 col-form-label text-md-right">{{ __('ชื่อจังหวัด') }}</label>
                    ...
                    <div class="m-2 p-2">
                        <form method="POST" action="{{ route('states.destroy', $state->id) }}">
                            @csrf
                            @method('DELETE')
                            <button class="btn btn-danger">(ลบ) {{ $state->name }}</button>
                        </form>
                    </div>
                </div>
            </div>
