            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">
                        {{ __('แก้ไขผู้ใช้') }}
                        <a href="{{ route('users.index') }}" class="float-right">ย้อนกลับ</a>
                    </div>
                ...
                <div class="m-2 p-2">
                    <form method="POST" action="{{ route('users.destroy', $user->id) }}">
                        @csrf
                        @method('DELETE')
                        <button class="btn btn-danger">(ลบ) {{ $user->username }}</button>
                    </form>
                </div>
            </div>
            <div class="col-md-8">
                        ...
                        <form method="POST" action="{{ route('users.change.password', $user->id) }}">
                            ...
                            <div class="form-group row">
                                <label for="password-confirm"
                                    class="col-md-4 col-form-label text-md-right">{{ __('ยืรยันรหัสผ่าน') }}</label>
